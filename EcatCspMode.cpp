...
public:
   ...
   const Error *Init( Node &node, uint16 slot=0x100 )
   {
      ...
      // are linked to.
      if( !err ) err = statusWord.Init( 0x6041+off, 0 );
      if( !err ) err = actualPos.Init( 0x6064+off, 0 );
      if( !err ) err = followingErr.Init( 0x60F4+off, 0 );
      if( !err ) err = actualVel.Init( 0x606C+off, 0 );
      if( !err ) err = actualTorque.Init( 0x6077+off, 0 );
...
int main( int argc, char **argv )
{
   const Error *err;
#if defined( WIN32 )
   WinUdpEcatHardware eth0( "192.168.0.98" );
#else
...
//Quick Stop Option Code (0x605A): 0;1;2;5;6
   ...
   err = ecat.SetSync0Period( &node, 1000000 );
